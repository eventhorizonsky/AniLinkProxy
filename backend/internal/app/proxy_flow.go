...
func endpointKey(path string) string { switch {
case strings.HasPrefix(path, "/api/v2/comment/"): return "comment"
case path == "/api/v2/search/episodes": return "search"
case strings.HasPrefix(path, "/api/v2/bangumi/shin"): return "shin"
case strings.HasPrefix(path, "/api/v2/bangumi/"): return "bangumi"
case path == "/api/v2/match": return "match"
case path == "/api/v2/match/batch": return "match_batch"
default: return "" } }

func isCacheableEndpoint(endpoint string) bool { return endpoint == "comment" || endpoint == "search" || endpoint == "bangumi" || endpoint == "shin" }
func cacheKeyOf(path string, q map[string][]string) string {
	keys := make([]string, 0, len(q)); for k := range q { keys = append(keys, k) }; sort.Strings(keys)
	var b strings.Builder; b.WriteString(path)
	for _, k := range keys { vals := append([]string(nil), q[k]...); sort.Strings(vals); for _, v := range vals { b.WriteString("|"); b.WriteString(k); b.WriteString("="); b.WriteString(v) } }
	return b.String()
}
func (s *APIServer) getTTL(endpoint string) int { if v, ok := s.getRuntime().CacheTTLMin[endpoint]; ok { return v }; return 30 }
func (s *APIServer) getRateLimit(endpoint string) EndpointLimit { if v, ok := s.getRuntime().RateLimit[endpoint]; ok { return v }; return EndpointLimit{RPS: 1, Burst: 2} }

func (s *APIServer) validateMatchPayload(endpoint string, body []byte) (string, string) {
	if len(body) == 0 { return "PAYLOAD_INVALID", "body is empty" }
	if endpoint == "match" {
		var req map[string]interface{}; if err := json.Unmarshal(body, &req); err != nil { return "PAYLOAD_INVALID", "invalid json body" }
		if _, ok := req["matchMode"]; !ok { req["matchMode"] = "hashAndFileName" }
		mode, _ := req["matchMode"].(string)
		if mode != "hashAndFileName" && mode != "hashOnly" && mode != "fileNameOnly" { return "PAYLOAD_INVALID", "unsupported matchMode" }
	} else {
		var req struct{ Requests []map[string]interface{} `json:"requests"` }
		if err := json.Unmarshal(body, &req); err != nil { return "PAYLOAD_INVALID", "invalid json body" }
		if len(req.Requests) == 0 { return "PAYLOAD_INVALID", "requests must not be empty" }
		if len(req.Requests) > s.getRuntime().BatchMaxItems { return "PAYLOAD_INVALID", "batch request exceeds max items" }
	}
	return "", ""
}

func (s *APIServer) tryAcquireMatchLock(appID string) (bool, func()) {
	s.matchMu.Lock(); defer s.matchMu.Unlock()
	if _, exists := s.matchLock[appID]; exists { return false, func() {} }
...
func (s *APIServer) cleanupMatchLoop() {
	...
	for range ticker.C { now := time.Now(); s.matchMu.Lock(); for appID, exp := range s.matchLock { if now.After(exp) { delete(s.matchLock, appID) } }; s.matchMu.Unlock() }
}
func (s *APIServer) recordMetric(appID, endpoint string, start time.Time, statusCode string) {
	if appID == "" { return }
	latency, date, now := time.Since(start).Milliseconds(), time.Now().Format("2006-01-02"), time.Now().UTC().Format(time.RFC3339)
	success, authFail, limited, upFail, timeout := 0, 0, 0, 0, 0
	switch statusCode { case "OK": success = 1; case "AUTH_SIGNATURE_INVALID", "AUTH_HEADER_MISSING", "AUTH_INVALID_APP", "AUTH_TIMESTAMP_INVALID", "AUTH_TIMESTAMP_EXPIRED": authFail = 1; case "RATE_LIMITED", "MATCH_IN_FLIGHT": limited = 1; case "UPSTREAM_TIMEOUT": timeout = 1; default: if strings.Contains(statusCode, "UPSTREAM") { upFail = 1 } }
	_, err := s.db.Exec(`INSERT INTO app_metrics_daily(app_id, endpoint, date, total, success, auth_fail, rate_limited, upstream_fail, timeout, total_latency_ms, updated_at)
		...
		ON CONFLICT(app_id, endpoint, date) DO UPDATE SET
			total=total+1, success=success+excluded.success, auth_fail=auth_fail+excluded.auth_fail, rate_limited=rate_limited+excluded.rate_limited, upstream_fail=upstream_fail+excluded.upstream_fail, timeout=timeout+excluded.timeout, total_latency_ms=total_latency_ms+excluded.total_latency_ms, updated_at=excluded.updated_at`,
		appID, endpoint, date, 1, success, authFail, limited, upFail, timeout, latency, now)
	if err != nil { log.Printf("record metric error: %v", err) }
}
func (s *APIServer) createRiskEvent(user User, level, rule string, metric float64, detail string) {
	now := time.Now().UTC().Format(time.RFC3339)
	_, _ = s.db.Exec(`INSERT INTO risk_events(app_id, user_id, level, rule_name, metric_value, detail, created_at) VALUES(?,?,?,?,?,?,?)`, user.AppID, user.ID, level, rule, metric, detail, now)
	if !s.getRuntime().AutoBanEnabled || level != "high" { return }
	banUntil := time.Now().Add(time.Duration(s.getRuntime().AutoBanMinutes) * time.Minute).UTC().Format(time.RFC3339)
	_, _ = s.db.Exec(`UPDATE users SET status='banned', ban_reason=?, ban_until=?, updated_at=? WHERE id=?`, "auto risk control ban", banUntil, now, user.ID)
}
